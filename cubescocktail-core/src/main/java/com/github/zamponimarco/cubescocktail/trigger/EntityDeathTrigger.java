...
public class EntityDeathTrigger extends CombatTrigger {
    ...
    private void legacyTransition(Map<String, Object> map) {
        ...
        List<Action> onKillerActions = (List<Action>) map.get("onKillerActions");
        if (onKillerActions != null && !onKillerActions.isEmpty()) {
            groups.add(new ActionGroup(new CasterSource(), new KillerTarget(), onKillerActions));
        }
    }
